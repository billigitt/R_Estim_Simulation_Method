function SimulationOutput = SimulationInference_ParallelEdit(SimulationInput)
...
% THIS FUNCTION IS AN EDIT TO THE ORIGINAL AS IT WANTS TO ALSO COUNT THE
% NUMBER OF SIMULATIONS REQUIRED TO GET M MATCHES. WE RECORD THIS THROUGH
% THE 'simsSoFar' VARIABLE.
...
%          spmd(Workers)
            ...
            while MoreMatchingPairs > 0
                ...
                simsSoFar(t) = simsSoFar(t) + MoreMatchingPairs;
...
s = 1;
        ...
        if t == 2
                ...
                I_disag_final((s-1)*MatchingPairs+1:s*MatchingPairs, 1:N) = I_disag_core;
                

            
        end
            
            I_disag_final(1+(s-1)*MatchingPairs:s*MatchingPairs, (t-1)*N+1:t*N) = I_disag_core;
            
            R_RawPosterior((s-1)*MatchingPairs+1:s*MatchingPairs, t) = R_RawPosterior_core;
...
SimulationOutput = struct('t', 2:T, 'Means', Means(2:end), 'CIs', CIs(:, 2:end)...
    , 'w', w, 'IncidenceDisagreggation', I_disag_final, 'totalSimulations', simsSoFar);
close(Waiting)
end
